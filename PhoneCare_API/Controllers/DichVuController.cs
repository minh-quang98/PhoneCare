...
namespace PhoneCare_API.Controllers
{
    ...
    public class DichVuController : ControllerBase
    {
        ...
        /// </summary>
        [HttpPut("{dichVuId:int}")]
        public async Task<ActionResult<ApiResponse<DichVuDto>>> Update(int dichVuId, UpdateDichVuDto request)
        {
            ...
            var item = await _db.DichVus.Include(x => x.DonHang).FirstOrDefaultAsync(x => x.Id == dichVuId && !x.IsDeleted);
        ...
        /// </summary>
        [HttpDelete("{dichVuId:int}")]
        public async Task<ActionResult<ApiResponse<object>>> Delete(int dichVuId)
        {
            var current = _currentUserService.GetCurrentUser(HttpContext);
